...
impl AppMod for ScyllaDB {
    ...
    async fn config(
        ...
        let hosts = c.get_or_panic::<String>("hosts").split(',').map(|s| s.to_string()).collect();
...
impl ScyllaDB {
    ...
    pub async fn prepare(&self, query: &String) -> Result<Arc<PreparedStatement>, MajordomeError> {
            ...
            None => {
                ...
                let r = match self.inner.cache.get(query) {
                    ...
                    None => {
                        ...
                        let prep = self.inner.db.prepare(query.clone()).await?;
    ...
    pub async fn prepare_by_hash_or(&self, hash: u64, query: impl FnOnce() -> String) -> Result<Arc<PreparedStatement>, MajordomeError> {
    ...
    pub async fn execute(&self, prepared: &PreparedStatement, values: impl SerializeRow) -> Result<scylla::QueryResult, MajordomeError> {
        let result = self.inner.db.execute(prepared, values).await?;
        Ok(result)
    }
    /// Execute a non-prepared query with values.
    /// Attempts to retrieve a prepared statement from the cache, or prepares it if it's not found.
    pub async fn query(&self, query: &String, values: impl SerializeRow) -> Result<scylla::QueryResult, MajordomeError> {
